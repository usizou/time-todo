...
タイマー機能を中心にしたデスクトップアプリ。現時点ではカウントダウン・ポモドーロ・時刻指定
（目標時刻までのカウントダウン）を備え、将来的に To-Do 機能を統合していく。
本資料は実装済みの設計と、今後の拡張方針をまとめる。
...
- 現在タイマーは状態を永続化していない（起動ごとに初期化）。To-Do 導入時に本レイヤーを本格利用する。
