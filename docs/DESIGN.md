...
- **「セット」ボタンは廃止**。時/分/秒・目標時刻は入力の `change` で即反映（`applyCustom`/`applyTarget`）。
- **稼働中は時間変更を不可**：`render()` で当該モードが `running` の間は時/分/秒・プリセット・目標時刻入力を `disabled` にする（一時停止中は変更可）。
...
- **タグ機能**（メモと同一方式）：本文の `#タグ` を `parseTags()` で `tags` に保持し、表示は `stripTags()` で除去してチップ表示。`#todo-filter` と `todoTagFilter` で絞り込み、絞り込み中の追加は該当タグを自動付与。通知・次の予定の本文も `stripTags()` 済み。CSVは既存スキーマのまま（本文にタグが残り、読み込み時に再抽出）
...
- **「時刻まで」モードでの次の予定表示**（`nextAlarmTodo`/`updateNextTodo`。時刻を設定した未完了タスクのうち次回発生が最も近いものをタイマー下部に表示。同モード時のみ）
  - 押すと To-Do タブへ移動し、該当タスクを一瞬強調（`flash`。`jumpToTodo()` 共通化）
  - 予定時刻を過ぎた未完了タスクは、別ブロック `#overdue-todo` に**2行**（「⚠ 未完了のタスク」＋時刻・本文）で**最も過ぎた1件**を表示（`firstOverdueTodo`/`updateOverdueTodo`）。「次の予定」に出ているタスクは除外。クリックで同様に該当タスクへリンク
  - 表示条件：**今日・未完了で、これから来るもの**のうち今に最も近い1件（翌日以降は翌日になってから）。予定時刻を過ぎたものは「次の予定」には出さず、下の `#overdue-todo` に回す
